...
L0, L1, L2 = 10, 10, 10
model = pickle.load(open("tanh_199.pickle", "br"))
...
def from_fen(board, color, castling, enpas, _hclock, _fclock):
    ...
    pos = Position(board, score, wf, bf, wc, bc, ep, 0)
    return pos if color == 'w' else pos.rotate(False)
...
def chess_bot(obs):
    ...
    for depth, gamma, score, move in searcher.search(hist):
        if score >= gamma:
            move = move
        if move and time.time() - start_time > 0.1:
            break
    i, j = move.i, move.j
    if color == 'b':
        i, j = 119 - i, 119 - j
...
if __name__ == '__main__':
    chess_bot("r2qrk2/p1p1b1pp/3p1n2/pN2p3/2Q1P3/P3B3/1PP2PPP/3RR1K1 b - - 3 17")
    import psutil
