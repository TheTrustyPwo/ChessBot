...
class Searcher:
    ...
    def search(self, history, max_depth):
        self.nodes = 0
...
def chess_bot(obs):
    ...
    if pieces_left >= 18: constraint, depth = 0.2, 4
    elif pieces_left >= 12: constraint, depth = 0.5, 6
    else: constraint, depth = 0.2, 6

    move = None
    for d, gamma, s, m in searcher.search([pos], depth):
        if s >= gamma:
            move = m
            if s >= 50710:
                break
...
if __name__ == '__main__':
    ...
    print(chess_bot("8/4k2p/p1p1P3/2pb1P2/6N1/3K4/1P5P/8 b - - 2 36"))
    import psutil
